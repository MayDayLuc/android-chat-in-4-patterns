...
import nju.androidchat.client.hw1.component.ItemPicReceive;
import nju.androidchat.client.hw1.component.ItemPicSend;
...
public class Hw1TalkActivity extends AppCompatActivity implements Hw1Contract.View, TextView.OnEditorActionListener, OnRecallMessageRequested {
    ...
    public void showMessageList(List<ClientMessage> messages) {
        runOnUiThread(() -> {
                    ...
                    for (ClientMessage message : messages) {
                        ...
                        content.addView(showImg(message, text));
                        }
    ...
    private LinearLayout showImg(ClientMessage message, String text){
        String pattern = "^!\\[.*?\\]\\((.*?)\\)$";
        Pattern r = Pattern.compile(pattern);
        Matcher m = r.matcher(text);
        boolean flag = m.find();
        String uri = flag ? m.group(1) : null;
        System.out.println(uri);
        if (message.getSenderUsername().equals(this.presenter.getUsername())) {
            if(flag){
                return new ItemPicSend(this, uri, message.getMessageId(), this);
            }
            else {
            ...
            if(flag){
                return new ItemPicReceive(this, uri, message.getMessageId());
            }
            else{
